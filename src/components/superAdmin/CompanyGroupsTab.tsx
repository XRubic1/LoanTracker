...
/** Legacy cross-owner client list linking (advanced). */
export function CompanyGroupsTab() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setGroups(await fetchCompanyGroups());
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-muted2 text-[13px] max-w-2xl">
        Link multiple team accounts so they share client lists on worksheets. Separate from
        provisioned companies above.
      </p>
      ...
      <form onSubmit={handleCreateGroup} className="flex flex-wrap gap-2 items-end">
